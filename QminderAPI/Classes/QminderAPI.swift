...
public class QminderAPI {
  ...
  */
  public class func pairTV(code:String, secret:String, completionHandler: @escaping (_ status:String?, _ apiKey:String?, _ location:Int?, _ error:Error?) -> Void) {
    ...
    Alamofire.request("\(self.SERVER)/tv/code/\(code)", parameters: ["secret": secret]).responseJSON {
      response in
        ...
        if parsedResponse.valid {
          completionHandler(parsedResponse.json?["status"].string, parsedResponse.json?["apiKey"].string, parsedResponse.json?["location"].int, nil)
        } else {
          completionHandler(nil, nil, nil, parsedResponse.error)
        }
    }
  }
